...
export function createBatchCommandController(dependencies) {
  ...
  async function requestCheckpoint(type, payload) {
    ...
    if (!response?.ok) {
      if (
        response?.error === 'batch_ownership_active' &&
        response.checkpoint?.batchId
      ) {
        publish({
  ...
  function backgroundOwnsCancellation() {
    return cancellationReason !== null && cancellationReason !== 'offline';
  }
  ...
  function start(draft) {
    return runCommand('start', async () => {
      ...
      const checkpoint = await requestCheckpoint('BATCH_SESSION_START', {
        ...
        plan: safeDraft?.plan,
        confirmation: safeDraft?.confirmation,
        settings: safeDraft?.settings
      });
      if (cancellationReason) {
        if (backgroundOwnsCancellation()) return checkpoint;
  ...
  function resume() {
    return runCommand('resume', async () => {
      ...
      if (cancellationReason) {
        if (backgroundOwnsCancellation()) return resumed;
        return persistPauseWithRecovery(
          resumed,
          cancellationReason,
          'resume'
  ...
  function retryPersistence() {
    return runCommand('retry-persistence', async () => {
      const checkpoint = recoveryPersistence?.checkpoint || currentCheckpoint();
      if (
        recoveryPersistence?.batchId !== checkpoint.batchId &&
        checkpoint.persistencePending !== true
      ) {
        throw codedError('recovery_persistence_not_pending');
      }
      return persistPause(checkpoint, 'runtime_error');
    });
  }
  ...
  function stop(confirmedRisk = false) {
    ...
    return runCommand('stop', async () => {
      ...
      try {
        ...
        await workerRuntime.disposeAfterBackgroundTeardown?.();
        recoveryPersistence = null;
        return stopped;
      } catch (error) {
        return recoverPersistedWorkerFailure({
  ...
  function beginTeardown(reason = 'page_teardown') {
    cancellationReason = reason;
  }
