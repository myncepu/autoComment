...
const PAGE_RUNTIME_MESSAGE_TYPES = new Set([
  ...
  'BATCH_WORKER_TAB_REMOVED'
]);
const LOCAL_DEBUG_PAGE_COMMAND = 'LOCAL_DEBUG_PAGE_COMMAND';
...
function commonRedirectHosts(hostname) {
  ...
  const likelyApex = labels.length === 2 ||
    (labels.length === 3 && COMMON_SECOND_LEVEL_SUFFIXES.has(suffix));
  return likelyApex
...
function createDraftStorage(storageArea) {
  function persistentDraft(draft) {
    const scrubbed = scrubSensitive(draft);
    if (!scrubbed?.outlinkSource) return scrubbed;
    return {
      ...scrubbed,
      outlinkSource: {
        ...scrubbed.outlinkSource,
        records: [],
        loading: false,
        error: ''
      }
  ...
  return {
    ...
    async set(draft) {
      await storageArea.set({ [BATCH_DRAFT_KEY]: persistentDraft(draft) });
    },
    async remove() {
...
export function createChromeBatchDependencies(
  chromeApi,
  { now = Date.now } = {}
) {
  ...
  function subscribeRuntimeMessages(listener) {
    const handleMessage = (message, sender) => {
      ...
      listener(scrubSensitive(message));
      return false;
    };
  ...
  function subscribeLocalDebugCommands(listener) {
    const handleMessage = (message, sender, sendResponse) => {
      if (
        !isTrustedBackgroundSender(sender, runtime) ||
        message?.type !== LOCAL_DEBUG_PAGE_COMMAND ||
      ...
      void Promise.resolve(listener({
        ...
        (response) => sendResponse(scrubSensitive(response)),
        (error) => sendResponse({
          ok: false,
          error: typeof error?.code === 'string'
            ? error.code
            : 'batch_page_command_failed'
        })
      );
      return true;
    };
  ...
  async function requestTargetPermissions(rawUrls) {
    const origins = [...new Set((rawUrls || []).flatMap((rawUrl) => {
      try {
        const parsed = new URL(rawUrl);
        if (!['http:', 'https:'].includes(parsed.protocol)) return [];
        return commonRedirectHosts(parsed.hostname).flatMap((hostname) => [
          `http://${hostname}/*`,
          `https://${hostname}/*`
        ]);
      } catch (_) {
    ...
    if (
      ...
      const error = new Error('batch_target_permission_unavailable');
      error.code = 'batch_target_permission_unavailable';
      throw error;
    }
    ...
    let granted;
    try {
      granted = await permissionsApi.request({ origins });
    } catch (_) {
      const error = new Error('batch_target_permission_unavailable');
      error.code = 'batch_target_permission_unavailable';
      throw error;
    ...
    if (granted !== true) {
      const error = new Error('batch_target_permission_denied');
      error.code = 'batch_target_permission_denied';
      throw error;
    }
    return true;
  }
  ...
  async function loadDomainConfig() {
    const response = await runtimeRequest('BATCH_GET_MANUAL_DEFAULT_CONFIG');
    if (!response?.ok || !response.config) {
      const error = new Error(response?.error || 'domain_config_unavailable');
      error.code = response?.error || 'domain_config_unavailable';
      throw error;
    }
    return scrubSensitive(response.config);
  }

  async function loadRecentSuccessUrls() {
    let response;
    try {
      response = await runtimeRequest('HISTORY_RECENT_SUCCESS_URLS', {
    ...
    return [...new Set(response.data.map(text).filter(Boolean))];
  }

  async function loadSuccessfulTargetStats() {
    let response;
    try {
      response = await runtimeRequest('OUTLINKS_SUCCESS_STATS');
    } catch (_) {
      const error = new Error('outlink_success_history_unavailable');
      error.code = 'outlink_success_history_unavailable';
      throw error;
    }
    if (!response?.ok || !Array.isArray(response.data)) {
      const error = new Error('outlink_success_history_unavailable');
