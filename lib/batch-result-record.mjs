...
export function buildBatchResult(task, outcome = {}, timing = {}) {
  ...
  const attempt = Number.isInteger(timing.attempt) && timing.attempt > 0
    ? timing.attempt
    : 1;
  const preview = normalizeBatchResultPreview(outcome.resultPreview);
  const timestamp = Number.isFinite(timing.timestamp)
    ? timing.timestamp
    : Date.now();
  return {
    ...
    timestamp,
    submittedAt: outcome.result === 'success'
      ? (
          Number.isFinite(timing.submittedAt)
            ? timing.submittedAt
            : timestamp
        )
      : null,
