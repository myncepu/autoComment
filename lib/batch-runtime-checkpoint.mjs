  ...
  try {
    ...
    if (Array.isArray(checkpoint?.results)) {
      for (const result of checkpoint.results) {
        const submittedAt = result?.result === 'success'
          ? (
              Number.isFinite(result.submittedAt)
                ? result.submittedAt
...
function createResultEntry(checkpoint, urlIndex, result, now, startedAt) {
  ...
  return {
    ...
    errorMessage: result.errorMessage
      ...
      : null,
    timestamp: now,
    submittedAt: result.result === 'success' ? now : null,
    ...
    if (checkpoint.version === 1) {
      ...
      for (const result of checkpoint.results) {
        ...
        result.errorMessage = typeof result.errorMessage === 'string' ||
          ...
          : null;
        result.submittedAt = result.result === 'success'
          ? (
              Number.isFinite(result.submittedAt)
                ? result.submittedAt
                : Number.isFinite(result.timestamp)
