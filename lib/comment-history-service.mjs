...
export function createCommentHistoryService({
  ...
  async function saveConfirmedSuccess(message) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          pendingCount = Object.keys(storageData)
            .filter((key) => key.startsWith(PENDING_PREFIX)).length;
        } catch (_) {
          // The exact item is already durable even if the count refresh fails.
        }
        return {
          historySaveStatus: 'queued',
          ...(Number.isInteger(pendingCount) ? { pendingCount } : {})
        };
      } catch (_) {
        return { historySaveStatus: 'failed' };
      }
    ...
    const pendingKey = `${PENDING_PREFIX}${bundle.comment.id}`;
    let stalePendingRemoved = false;
    try {
      await storageLocal.remove(pendingKey);
      stalePendingRemoved = true;
    } catch (_) {
      // A durable IndexedDB write must not be downgraded by queue maintenance.
    }
    if (!stalePendingRemoved) {
      let pendingCount;
      try {
        const storageData = await storageLocal.get(null);
        pendingCount = Object.keys(storageData)
        ...
        // The fresh record is durable even when queue visibility is unavailable.
      }
      return {
    ...
    return {
      historySaveStatus: 'saved',
      ...(Number.isInteger(retryResult?.pending)
  ...
  async function retryPendingWrites({ limit } = {}) {
    ...
    for (const { key, bundle } of retryable) {
      retried += 1;
      try {
        const inserted = await repository.upsertPendingUnlessLive(bundle);
    ...
    return {
      ...
      pending: pendingKeys.length - saved - discarded
    };
  }
