...
export const OUTLINK_RECORD_MAX_PAGE_SIZE = 2000;
  ...
  return {
    ...
    async list({
      ...
      const records = annotateOutlinkRecords(
        await allMatching(filter),
        successStats
      ).sort((left, right) => (
        compareOutlinksBySuccess(left, right) || compareNewest(left, right)
      ));
      const safeOffset = Number.isInteger(offset) && offset > 0 ? offset : 0;
      const safeLimit = Number.isInteger(limit) && limit > 0
        ? Math.min(limit, OUTLINK_RECORD_MAX_PAGE_SIZE)
        : 100;
      return {
    ...
    async exportRecords(filter = {}, successStats = []) {
      return annotateOutlinkRecords(
        await allMatching(filter),
        successStats
      ).sort((left, right) => (
        compareOutlinksBySuccess(left, right) || compareNewest(left, right)
