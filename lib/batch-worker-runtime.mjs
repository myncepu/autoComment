...
export function createBatchWorkerRuntime(dependencies) {
  ...
  async function terminalizeOpening(owner, reservation, kind) {
    return claimFinalizer(owner, reservation, {
      reason: kind,
      skipSeal: true,
      alreadyClosed: true,
  ...
  async function open(urlIndex, owner) {
    ...
    if (!task || !item || hasUrlCredentials(item.url)) {
    ...
    const run = (async () => {
      ...
      const activeResponse = await transition(owner, 'BATCH_TASK_ACTIVE', {
        batchId: activity.batchId,
        urlIndex,
        attempt: activity.attempt,
        tabId: activity.tabId,
      ...
      try {
        ...
        const handleResponse = await sendHandle({
          ...activity,
          url: sanitizeBatchUrl(activity.url)
        });
  ...
  async function start(checkpoint) {
    ...
    if (previous) {
      ...
      if (!stopped) return previous.checkpoint;
  ...
  async function stop() {
    ...
    if (owner.status === 'running') await pause('stop');
    if (lifecycle === owner && owner.status !== 'paused_recovery') {
  ...
  async function dispose() {
    ...
    if (stopped && lifecycle === owner) lifecycle = null;
    return owner.checkpoint;
  }
