...
export function createBatchWorkerRuntime(dependencies) {
  ...
  function hasExactTaskIdentity(currentTask, nextTask, version) {
    if (
  ...
  async function acceptRemovedTabCheckpoint(message) {
    return enqueueLifecycle(async () => {
      ...
      if (
        ...
        !activity ||
        activity.batchId !== message.batchId ||
        activity.urlIndex !== urlIndex ||
        activity.attempt !== message.attempt ||
        activity.tabId !== message.tabId ||
      ...
      if (forgotten !== activity) return false;
      owner.deadlines?.clear(activity);
      owner.checkpoint = nextCheckpoint;
      reconcileScheduler(owner);
      emit('changed', owner, {
