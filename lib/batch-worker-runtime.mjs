...
export function isForwardRuntimeCheckpoint(
  currentCheckpoint,
  nextCheckpoint
) {
  return Boolean(
    currentCheckpoint &&
    nextCheckpoint &&
    nextCheckpoint.batchId === currentCheckpoint.batchId &&
    ...
    nextCheckpoint.createdAt === currentCheckpoint.createdAt &&
    ...
    (
      ...
      (
        nextCheckpoint.configRevision ===
          currentCheckpoint.configRevision &&
    ...
    Number(nextCheckpoint.cursor?.nextIndex) >=
      Number(currentCheckpoint.cursor?.nextIndex)
  );
}
...
function isWorkerPendingUrl(candidate) {
  if (typeof candidate !== 'string' || !candidate) return false;
  try {
    const parsed = new URL(candidate);
    return (
      parsed.protocol === 'chrome-extension:' &&
      parsed.pathname === '/worker-pending.html'
    );
  } catch (_) {
    return false;
  }
...
function unavailableReasonForTab(tab) {
  ...
  try {
    const parsed = new URL(candidate);
    if (isWorkerPendingUrl(candidate)) {
      // A freshly-created worker can still report the extension-owned
      // placeholder for a short interval after tabs.update() resolves and
      // before Chrome exposes pendingUrl. This is a navigation transition,
      // not a restricted target page; the readiness deadline remains the
      // authoritative bound if the target navigation never commits.
      return null;
    }
...
export function waitForContentScriptReady(activity, {
  ...
  return new Promise((resolve, reject) => {
    ...
    const fail = (reason) => {
      if (
        deadlineClaimed &&
        !['timeout', 'runtime_initialization_failed'].includes(reason)
      ) {
        return;
      }
    ...
    const probe = async () => {
      ...
      try {
        ...
        if (
          !state.tab?.pendingUrl &&
          isWorkerPendingUrl(state.tab?.url)
        ) {
          return;
        }
        ...
        try {
          ...
          if (deadlineClaimed) return;
          if (response?.bootstrapReady === true) {
            state.bootstrapSeen = true;
          }
          if (response?.ok === true) {
            if (
...
export function createBatchWorkerRuntime(dependencies) {
  ...
  async function transition(owner, type, payload, recovery = null) {
    try {
      ...
      if (!response?.ok) {
        const currentTask = owner.checkpoint.tasks?.[String(payload?.urlIndex)];
        const terminalRaceConverged =
  ...
  function acceptBackgroundCheckpoint(owner, activity) {
    ...
    if (
      ...
      !isForwardRuntimeCheckpoint(owner.checkpoint, checkpoint)
    ) {
      return false;
    }
  ...
  function claimFinalizer(owner, activity, options) {
    ...
    const hardDeadline = options.reason === 'timeout';
    const execute = () => performFinalizer(owner, activity, options);
    const execution = hardDeadline
  ...
  async function handleConfirmation(message) {
    ...
    if (message.checkpoint) {
      const reconciled = await reconcile(message.checkpoint);
  ...
  async function acceptRemovedTabCheckpoint(message) {
    return enqueueLifecycle(async () => {
      ...
      if (message?.deadlineExpired === true) {
        if (lifecycle !== owner || owner.status !== 'running') return false;
        ...
        owner.finalizers.delete(removalKey);
        if (owner.finalizingByIndex.get(urlIndex) === removalKey) {
          owner.finalizingByIndex.delete(urlIndex);
        }
        owner.checkpoint = message.checkpoint;
        reconcileScheduler(owner);
        emit('changed', owner, {
          transition: 'BATCH_TASK_DEADLINE_EXPIRED',
          recovered: true
        });
        await replenishOrComplete(owner);
        return true;
  ...
  async function recoverCapacity(checkpoint) {
    const owner = lifecycle;
    if (
      !owner ||
      !['running', 'paused_recovery'].includes(owner.status) ||
    ...
    if (owner.capacityRecovery) return owner.capacityRecovery;
    ...
    const recovery = (async () => {
      ...
      for (let urlIndex = 0; urlIndex < totalCount; urlIndex += 1) {
        const task = checkpoint.tasks?.[String(urlIndex)];
        const activity = activityAt(owner, urlIndex);
        const activityStillOwned = Boolean(
          activity &&
          ['active', 'submitting'].includes(task?.state) &&
          activity.attempt === task.attempt &&
          activity.tabId === task.tabId
        );
        if (activity && !activityStillOwned) {
          owner.deadlines?.clear(activity);
          owner.manager.forgetRemoved?.(activity.tabId);
          void Promise.resolve()
            .then(() => tabsApi.remove(activity.tabId))
            .catch(() => {});
          recoveredCount += 1;
        }
        ...
        if (
          ...
          (
            !task ||
            task.state === 'terminal' ||
            task.attempt !== opening.attempt
          )
        ) {
          cancelOpeningReservation(owner, opening);
          owner.deadlines?.clear(opening);
          recoveredCount += 1;
        }

        const finalizerKey = owner.finalizingByIndex.get(urlIndex);
        if (
          finalizerKey &&
          (
            !task ||
            task.state === 'terminal' ||
            !finalizerKey.endsWith(`:${task.attempt}`)
          )
        ) {
          owner.finalizingByIndex.delete(urlIndex);
          owner.finalizers.delete(finalizerKey);
          recoveredCount += 1;
        }
      }

      for (const [key, activity] of owner.removedActivities) {
        const task = checkpoint.tasks?.[String(activity.urlIndex)];
        if (
          !task ||
          task.state === 'terminal' ||
          ...
          owner.removedActivities.delete(key);
          owner.deadlines?.clear(activity);
          recoveredCount += 1;
        }
      }
      ...
      reconcileScheduler(owner);
      emit('changed', owner, {
        transition: 'BATCH_AUTHORITATIVE_CAPACITY_RECOVERY',
        recovered: true,
        reconciledCount: recoveredCount
      });
      await replenishOrComplete(owner);
      return true;
    })();
    owner.capacityRecovery = recovery;
    try {
      return await recovery;
    } finally {
      if (owner.capacityRecovery === recovery) {
        owner.capacityRecovery = null;
      }
    }
  }

  async function reconcile(checkpoint) {
    const owner = lifecycle;
    if (
      !owner ||
      !['running', 'paused_recovery'].includes(owner.status) ||
    ...
    return enqueue(owner, async () => {
      ...
      for (let urlIndex = 0; urlIndex < totalCount; urlIndex += 1) {
        const activity = activityAt(owner, urlIndex);
        const nextTask = checkpoint.tasks?.[String(urlIndex)];
        const removedActivity = Number.isInteger(nextTask?.attempt)
          ? owner.removedActivities.get([
              owner.checkpoint.batchId,
              urlIndex,
              nextTask.attempt
            ].join(':')) || null
          : null;
        ...
        if (nextTask?.state !== 'terminal') continue;
        if (!isForwardRemovedTabCheckpoint(owner.checkpoint, {
          batchId: owner.checkpoint.batchId,
          urlIndex,
        ...
        terminalActivities.push({
          activity: evidence,
          alreadyRemoved: Boolean(removedActivity && !activity)
        });
      }
      if (terminalActivities.length === 0) return false;
      owner.checkpoint = checkpoint;
      for (const { activity, alreadyRemoved } of terminalActivities) {
        const key = identityKey(activity);
        if (alreadyRemoved) {
          owner.removedActivities.delete(key);
          owner.deadlines?.clear(activity);
        ...
        if (!await closeActivity(owner, activity)) return false;
      }
      owner.status = 'running';
      owner.recoveryPause = null;
      startTimeoutChecker(owner);
      reconcileScheduler(owner);
      emit('changed', owner, {
        transition: 'BATCH_AUTHORITATIVE_RECONCILE',
        recovered: true,
        reconciledCount: terminalActivities.length
      });
      await replenishOrComplete(owner);
      return true;
    });
