...
function isStaleAttempt(context, savedContext) {
  return (
    context?.batchId === savedContext.batchId
    && context?.urlIndex === savedContext.urlIndex
    && Number.isInteger(context?.attempt)
    && context.attempt > savedContext.attempt
  );
}
  ...
  return {
    save(tabId, context) {
      return enqueue(async () => {
        if (!hasCompleteTaskIdentity(context)) {
...
export function installBatchSubmitContextListener(
  ...
  chromeApi.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    const identity = message.type === 'BATCH_SAVE_SUBMIT_CONTEXT'
      ? message.context
      : message.match;
