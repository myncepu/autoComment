...
} from './options-page-reliability.mjs';
...
function messageFor(error) {
  return optionsErrorMessage(error, '未知错误，请重试。');
}
...
export function createOptionsConfigBundleView({
  ...
  reportDiagnostic = (code, phase) => {
    console.error(`[options-config] ${phase}:`, code);
  }
}) {
  ...
  for (const liveRegion of [elements.status, elements.summary]) {
  ...
  function report(code, phase) {
    try {
      reportDiagnostic(code, phase);
    } catch (_) {
      // Diagnostics are best effort and must not interrupt recovery controls.
    }
  }
  ...
  function renderPreview(preview) {
    ...
    const conflictCodes = Array.isArray(preview?.conflicts)
      ? preview.conflicts.map(({ code }) => stableOptionsErrorCode(
          { code },
          'config_bundle_conflict'
        ))
      : [];
    const conflictMessages = conflictCodes.map((code) => (
      optionsErrorMessage(
        { code },
        '存在无法处理的配置冲突，请检查文件后重试。'
      )
    ));
    for (const code of conflictCodes) {
      report(code, 'preview-conflict');
    }
    const summary = [
      `新增 ${created}`,
      `更新 ${updated}`,
    ...
    if (conflictMessages.length > 0) {
      summary.push(`冲突 ${conflictMessages.join('、')}`);
    }
  ...
  function handleExport() {
    void run(async () => {
      try {
        ...
        if (!destroyed) {
          report(
            stableOptionsErrorCode(error, 'config_bundle_export_failed'),
            'export'
          );
    ...
    void run(async () => {
      try {
        ...
        try {
          ...
          throw codedError('invalid_config_bundle_json');
        }
        ...
        if (!destroyed) {
          report(
            stableOptionsErrorCode(error, 'config_bundle_import_failed'),
            'import'
          );
          clearPreview();
  ...
  function handleApply() {
    ...
    void run(async () => {
      ...
      try {
        ...
        if (!destroyed) {
          report(
            stableOptionsErrorCode(error, 'config_bundle_apply_failed'),
            'apply'
          );
          clearPreview();
      ...
      try {
        ...
        if (!destroyed) {
          report(
            stableOptionsErrorCode(error, 'options_config_refresh_failed'),
            'refresh'
          );
          renderStatus(
            '配置已应用，但页面刷新失败；请重新加载',
