...
export function preflightBatchRows(document, dependencies) {
  ...
  const rows = (document.rows || []).map((originalRow, index) => {
    ...
    const rawUrl = urlIndex >= 0 ? originalRow[urlIndex] : '';
    const candidate = /^https?:\/\//i.test(String(rawUrl || '').trim())
      ? String(rawUrl || '').trim()
      : `https://${String(rawUrl || '').trim()}`;
    const containsCredentials = hasUrlCredentials(candidate);
    const url = containsCredentials ? null : asHttpUrl(rawUrl);
    const safeOriginalRow = [...originalRow];
    if (urlIndex >= 0 && containsCredentials) {
      safeOriginalRow[urlIndex] = sanitizeBatchUrl(candidate);
    } else if (urlIndex >= 0 && url) {
      safeOriginalRow[urlIndex] = url;
    }
    ...
    const base = {
      rowNumber,
      originalRow: safeOriginalRow,
      url,
      sourceDomain,
      status: 'invalid',
      reasonCode: 'invalid_url',
      reason: containsCredentials ? 'URL 不得包含用户名或密码凭证' : 'URL 无效',
      overridable: false,
      included: false
    };
