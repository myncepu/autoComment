const SENSITIVE_KEYS = new Set([
  'token',
  'accesstoken',
  'refreshtoken',
  'idtoken',
  'apikey',
  'clientsecret',
  'key',
  'password',
  'passwd',
  'secret',
  'auth',
  'authorization'
]);

function normalizedKey(value) {
  return String(value || '').toLowerCase().replace(/[^a-z0-9]/g, '');
}

function isSensitiveKey(value) {
  return SENSITIVE_KEYS.has(normalizedKey(value));
}

function redactParams(params) {
  for (const key of [...params.keys()]) {
    if (isSensitiveKey(key)) params.set(key, 'REDACTED');
  }
}

const SENSITIVE_KEY_PATTERN = [
  'token',
  'access[_-]?token',
  'refresh[_-]?token',
  'id[_-]?token',
  'api[_-]?key',
  'client[_-]?secret',
  'key',
  'password',
  'passwd',
  'secret',
  'auth',
  'authorization'
].join('|');

const JSON_STRING_PATTERN = String.raw`"(?:\\.|[^"\\])*"`;
const JSON_STRING_PROPERTY = new RegExp(
  `(${JSON_STRING_PATTERN})(\\s*:\\s*)(${JSON_STRING_PATTERN})`,
  'g'
);

function sanitizeHash(hash) {
  const raw = String(hash || '').replace(/^#/, '');
  const queryAt = raw.indexOf('?');
  if (queryAt >= 0) {
    const route = raw.slice(0, queryAt);
    const params = new URLSearchParams(raw.slice(queryAt + 1));
    redactParams(params);
    return `${route}?${params.toString()}`;
  }
  if (!raw.includes('=')) return raw;
  const params = new URLSearchParams(raw);
  redactParams(params);
  return params.toString();
}

export function hasUrlCredentials(value) {
  try {
    const url = new URL(String(value || ''));
    return url.username.length > 0 || url.password.length > 0;
  } catch (_) {
    return false;
  }
}

export function sanitizeBatchUrl(value) {
  const raw = String(value || '');
  try {
    const url = new URL(raw);
    url.username = '';
    url.password = '';
    redactParams(url.searchParams);
    if (url.hash) {
      url.hash = sanitizeHash(url.hash);
    }
    return url.href;
  } catch (_) {
    return sanitizeDiagnosticText(raw);
  }
}

export function sanitizeDiagnosticText(value) {
  let text = String(value || '');
  const sanitizedUrls = [];
  text = text.replace(/https?:\/\/[^\s"'<>;,}]+/gi, (candidate) => {
    const marker = `\uE000${sanitizedUrls.length}\uE001`;
    try {
      sanitizedUrls.push(sanitizeBatchUrl(candidate));
    } catch (_) {
      sanitizedUrls.push(candidate.replace(
        /^(https?:\/\/)[^/@\s]+@/i,
        '$1'
      ));
    }
    return marker;
  });
  text = text.replace(
    JSON_STRING_PROPERTY,
    (match, encodedKey, separator) => {
      try {
        if (!isSensitiveKey(JSON.parse(encodedKey))) return match;
      } catch (_) {
        return match;
      }
      return `${encodedKey}${separator}"REDACTED"`;
    }
  );
  text = text.replace(
    /\b(authorization)\s*([=:])\s*(?:Bearer|Basic)\s+[^\s;,}]+/gi,
    (_match, key, separator) => `${key}${separator} REDACTED`
  );
  text = text.replace(
    /\b(Bearer|Basic)\s+[^\s;,}]+/gi,
    (_match, scheme) => `${scheme} REDACTED`
  );
  const rawAssignment = new RegExp(
    `\\b(${SENSITIVE_KEY_PATTERN})\\s*([=:])\\s*([^\\s;&,}]+)`,
    'gi'
  );
  text = text.replace(
    rawAssignment,
    (_match, key, separator) => `${key}${separator}REDACTED`
  );
  return text.replace(/\uE000(\d+)\uE001/g, (_match, index) => (
    sanitizedUrls[Number(index)] || ''
  ));
}
