...
function defaultExportResults(documentRef, checkpoint, legacyResults) {
...
export async function bootBatchPage(documentRef, dependencies) {
  if (!documentRef?.querySelector) throw codedError('batch_document_missing');
  requireDependencies(dependencies);
  const shellMount = bootAppShell(documentRef, {
  ...
  consoleView = createBatchConsoleView(documentRef, {
    ...
    onExport() {
      const exportResults = dependencies.exportResults ||
        ((currentCheckpoint, fallback) => defaultExportResults(
          documentRef,
          currentCheckpoint,
          fallback
        ));
