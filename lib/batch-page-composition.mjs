...
function hasMatchingRemovedTaskIdentity(currentTask, nextTask, version) {
  if (
...
function canAdoptRejectedRemovedCheckpoint(current, message) {
  ...
  const currentTask = current?.tasks?.[String(urlIndex)];
  const nextTask = next?.tasks?.[String(urlIndex)];
  return (
    typeof message?.batchId === 'string' &&
    message.batchId === current?.batchId &&
    ...
    hasMatchingRemovedTaskIdentity(currentTask, nextTask, current.version)
  );
}
