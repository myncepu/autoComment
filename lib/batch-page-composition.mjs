...
async function bootBatchPageInstance(documentRef, dependencies) {
  ...
  let planningAvailabilityError =
    domainConfigLoad.error || recentSuccessLoad.error || '';
  if (
  ...
  const createPlanControllerFor = (config, successUrls) => (
  ...
  if (planningAvailabilityError) {
    ...
    try {
      planController = createPlanControllerFor(domainConfig, recentSuccessUrls);
    } catch (_) {
      planningAvailabilityError = 'domain_config_unavailable';
    }
    if (planController) {
  ...
  async function retryPlanningLoad(currentDraft) {
    const [nextDomainConfigLoad, nextRecentSuccessLoad] = await Promise.all([
      loadPlanningDependency(
        dependencies.loadDomainConfig,
        null,
        'domain_config_unavailable'
    ...
    planController = domainConfig
      ? createPlanControllerFor(domainConfig, recentSuccessUrls)
      : null;
    wizardDraft = {
      ...currentDraft,
      availabilityError: '',
