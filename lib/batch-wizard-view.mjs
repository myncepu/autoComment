...
function clonePreflight(preflight) {
  ...
  return {
    headers: Array.isArray(preflight.headers) ? [...preflight.headers] : [],
    rows: preflight.rows.map((row) => ({
      ...row,
      originalRow: Array.isArray(row.originalRow) ? [...row.originalRow] : row.originalRow
    })),
    summary: {
      raw: Number(preflight.summary?.raw) || 0,
      eligible: Number(preflight.summary?.eligible) || 0,
      duplicate: Number(preflight.summary?.duplicate) || 0,
      blocked: Number(preflight.summary?.blocked) || 0,
      invalid: Number(preflight.summary?.invalid) || 0,
      included: Number(preflight.summary?.included) || 0
    }
  };
}
...
function section(documentRef, step, label) {
  ...
  appendText(documentRef, item, 'h2', label, { id: headingId });
  return item;
}
...
export function createBatchWizardView(documentRef, handlers = {}) {
  ...
  function updateDraft(nextDraft, { emit = true } = {}) {
  ...
  function onClick(event) {
    ...
    if (action === 'wizard-back' && draft.step > 1) {
      updateDraft({ ...draft, step: draft.step - 1 });
      return;
    }
    if (action === 'wizard-next' && stepIsReady(
      ...
      updateDraft({ ...draft, step: Math.min(4, draft.step + 1) });
      return;
    }
