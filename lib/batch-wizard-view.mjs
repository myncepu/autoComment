...
import { normalizeBatchOutlinkRecords } from './batch-outlink-source.mjs';
import { batchSkipReasonLabel } from './batch-result-classification.mjs';
import {
  sanitizeBatchUrl,
  ...
  return {
    ...
    promotionSites: config.promotionSites.map((site) => ({
      ...
      pages: Array.isArray(site?.pages)
        ? site.pages.map((page) => ({
            id: text(page?.id),
            url: sanitizeBatchUrl(page?.url),
            keywords: Array.isArray(page?.keywords)
              ? page.keywords.map(text).filter(Boolean)
              : [],
...
function safeOutlinkSource(source = {}) {
  return {
    query: sanitizeDiagnosticText(text(source.query)),
    page: clampInteger(source.page, 0, 100_000, 0),
    pageSize: clampInteger(source.pageSize, 1, 200, 50),
    total: clampInteger(source.total, 0, 10_000_000, 0),
    records: normalizeBatchOutlinkRecords(source.records),
    selectedRecords: normalizeBatchOutlinkRecords(source.selectedRecords),
    loading: source.loading === true,
    error: sanitizeDiagnosticText(text(source.error))
  };
}
...
function normalizeDraft(source = {}) {
  return {
    ...
    selectedProfileIds: Array.isArray(source.selectedProfileIds)
      ? source.selectedProfileIds.map(text).filter(Boolean)
      : [],
    selectedPromotionPageIds: Array.isArray(source.selectedPromotionPageIds)
      ? source.selectedPromotionPageIds.map(text).filter(Boolean)
      : [],
    allocationSelectionInitialized: source.allocationSelectionInitialized === true,
    ...
    availabilityError: text(source.availabilityError),
    readinessError: text(source.readinessError),
    parseError: text(source.parseError),
    sourceMode: source.sourceMode === 'outlinks' ? 'outlinks' : 'csv',
    outlinkSource: safeOutlinkSource(source.outlinkSource)
  };
}
...
function domainAssignmentIsReady(config) {
  ...
  const modernPages = config.promotionSites.flatMap(
    (site) => (site.enabled ? site.pages || [] : [])
...
function draftAssignmentIsReady(draft) {
  const modernPages = draft.domainConfig?.promotionSites?.flatMap(
    (site) => site.pages || []
  ) || [];
  if (modernPages.length > 0 && draft.allocationSelectionInitialized) {
    return draft.selectedProfileIds.length > 0
      && draft.selectedPromotionPageIds.length > 0;
  }
...
function renderAssignmentStep(documentRef, draft, availabilityError) {
  const item = section(documentRef, 1, STEP_LABELS[0]);
  if (availabilityError) {
    const alert = documentRef.createElement('div');
    alert.className = 'batch-wizard__error';
    alert.setAttribute('role', 'alert');
    alert.dataset.planningAvailabilityError = '';
    appendText(documentRef, alert, 'p', availabilityError);
    const retry = appendText(
      documentRef,
      alert,
      'button',
      '重试加载批次配置',
      {
        type: 'button',
        'data-action': 'retry-planning-load'
      }
    );
    retry.className = 'batch-wizard__button batch-wizard__button--secondary';
    item.appendChild(alert);
  }
  if (draft.domainConfig) {
    const modernPages = draft.domainConfig.promotionSites.flatMap((site) => (
      site.enabled
        ? (site.pages || []).filter(({ enabled }) => enabled).map((page) => ({
            ...page,
            siteName: site.name
          }))
        : []
    ...
    if (modernPages.length > 0) {
      appendText(
        ...
        item,
        'p',
        '选择本批次使用的身份和推广页面；两组会分别轮流分配，不再使用 Pair 或权重。'
      );
      const identityGroup = documentRef.createElement('fieldset');
      identityGroup.className = 'batch-wizard__selection-group';
      appendText(documentRef, identityGroup, 'legend', '身份（可多选）');
      const selectedProfiles = new Set(
        draft.allocationSelectionInitialized
          ? draft.selectedProfileIds
          : draft.domainConfig.profiles.map(({ id }) => id)
      );
      for (const profile of draft.domainConfig.profiles) {
        const label = documentRef.createElement('label');
        const input = documentRef.createElement('input');
        input.type = 'checkbox';
        input.name = 'selectedProfileIds';
        input.value = profile.id;
        input.checked = selectedProfiles.has(profile.id);
        label.append(input, documentRef.createTextNode(profile.displayName));
        identityGroup.appendChild(label);
      }
      const pageGroup = documentRef.createElement('fieldset');
      pageGroup.className = 'batch-wizard__selection-group';
      appendText(documentRef, pageGroup, 'legend', '推广页面（可多选）');
      const selectedPages = new Set(
        draft.allocationSelectionInitialized
          ? draft.selectedPromotionPageIds
          : modernPages.map(({ id }) => id)
      );
      for (const page of modernPages) {
        const label = documentRef.createElement('label');
        const input = documentRef.createElement('input');
        input.type = 'checkbox';
        input.name = 'selectedPromotionPageIds';
        input.value = page.id;
        input.checked = selectedPages.has(page.id);
        label.append(
          input,
          documentRef.createTextNode(`${page.siteName} · ${page.url}`)
        );
        pageGroup.appendChild(label);
      }
      item.append(identityGroup, pageGroup);
      return item;
    }
    appendText(
      documentRef,
...
function renderPlanTable(documentRef, draft) {
  ...
  for (const task of draft.plan.tasks) {
    ...
    appendText(
      ...
      task.state === 'eligible'
        ? '可执行'
        : batchSkipReasonLabel(task.blockReason)
    );
...
function renderSourceChoice(documentRef, draft) {
  const fieldset = documentRef.createElement('fieldset');
  fieldset.className = 'batch-wizard__source-choice';
  appendText(documentRef, fieldset, 'legend', '选择目标网站来源');
  for (const [value, label] of [
    ['csv', '上传 CSV 文件'],
    ['outlinks', '从已保存外链中选择']
  ]) {
    const option = documentRef.createElement('label');
    option.className = 'batch-wizard__source-option';
    const radio = documentRef.createElement('input');
    radio.type = 'radio';
    radio.name = 'sourceMode';
    radio.value = value;
    radio.checked = draft.sourceMode === value;
    option.append(radio, documentRef.createTextNode(label));
    fieldset.appendChild(option);
  }
  return fieldset;
}

function renderOutlinkPicker(documentRef, draft) {
  const source = draft.outlinkSource;
  const fragment = documentRef.createDocumentFragment();
  const search = documentRef.createElement('div');
  search.className = 'batch-wizard__outlink-search';
  const label = appendText(documentRef, search, 'label', '搜索已保存外链', {
    for: 'batch-wizard-outlink-query'
  });
  label.className = 'sr-only';
  const input = documentRef.createElement('input');
  input.id = 'batch-wizard-outlink-query';
  input.name = 'outlinkQuery';
  input.type = 'search';
  input.placeholder = '搜索目标网址、域名或来源页面';
  input.value = source.query;
  const submit = appendText(documentRef, search, 'button', '搜索', {
    type: 'button',
    'data-action': 'outlink-search'
  });
  submit.className = 'batch-wizard__button batch-wizard__button--secondary';
  submit.disabled = source.loading;
  search.insertBefore(input, submit);
  fragment.appendChild(search);

  const toolbar = documentRef.createElement('div');
  toolbar.className = 'batch-wizard__outlink-toolbar';
  appendText(
    documentRef,
    toolbar,
    'p',
    `已选择 ${source.selectedRecords.length} 个网站；当前搜索共 ${source.total} 条记录。`,
    { 'data-outlink-selection-summary': '', 'aria-live': 'polite' }
  );
  const selectPage = appendText(documentRef, toolbar, 'button', '选择当前页', {
    type: 'button',
    'data-action': 'outlink-select-page'
  });
  selectPage.className = 'batch-wizard__text-action';
  selectPage.disabled = source.loading || source.records.length === 0;
  const clear = appendText(documentRef, toolbar, 'button', '清空选择', {
    type: 'button',
    'data-action': 'outlink-clear-selection'
  });
  clear.className = 'batch-wizard__text-action';
  clear.disabled = source.loading || source.selectedRecords.length === 0;
  fragment.appendChild(toolbar);

  if (source.error || draft.parseError) {
    appendText(documentRef, fragment, 'p', source.error || draft.parseError, {
      class: 'batch-wizard__error',
      role: 'alert',
      'data-outlink-error': ''
    });
  } else if (source.loading) {
    appendText(documentRef, fragment, 'p', '正在读取已保存的外链…', {
      class: 'batch-wizard__empty',
      'data-outlink-loading': ''
    });
  } else if (source.records.length === 0) {
    appendText(documentRef, fragment, 'p', '没有找到可选择的已保存外链。', {
      class: 'batch-wizard__empty',
      'data-outlink-empty': ''
    });
  } else {
    const selectedUrls = new Set(source.selectedRecords.map(({ url }) => url));
    const wrapper = documentRef.createElement('div');
    wrapper.className = 'batch-wizard__table-wrap';
    wrapper.setAttribute('aria-label', '已保存外链选择列表，可横向滚动');
    const table = documentRef.createElement('table');
    table.className = 'batch-wizard__table batch-wizard__outlink-table';
    const thead = documentRef.createElement('thead');
    const headerRow = documentRef.createElement('tr');
    for (const heading of ['选择', '目标网站', '成功记录', '来源页面', '类型']) {
      appendText(documentRef, headerRow, 'th', heading, { scope: 'col' });
    }
    ...
    for (const record of source.records) {
      const row = documentRef.createElement('tr');
      row.dataset.outlinkUrl = record.url;
      const selectionCell = documentRef.createElement('td');
      const checkbox = documentRef.createElement('input');
      checkbox.type = 'checkbox';
      checkbox.name = 'outlinkRecordSelection';
      checkbox.value = record.url;
      checkbox.checked = selectedUrls.has(record.url);
      checkbox.setAttribute('aria-label', `选择 ${record.host || record.url}`);
      selectionCell.appendChild(checkbox);
      row.appendChild(selectionCell);
      const targetCell = documentRef.createElement('td');
      ...
      target.title = record.url;
      row.appendChild(targetCell);
      const successCell = documentRef.createElement('td');
      if (record.successCount > 0) {
        ...
        badge.className = 'batch-wizard__success-count';
        if (record.successfulPromotedDomains.length > 0) {
          badge.title = `已成功推广：${record.successfulPromotedDomains.join('、')}`;
        }
      } else {
        successCell.textContent = '—';
      }
      row.appendChild(successCell);
      appendText(documentRef, row, 'td', record.sourceHost || '—');
      appendText(documentRef, row, 'td', record.isDofollow ? 'DoFollow' : 'NoFollow');
  ...
  pagination.className = 'batch-wizard__outlink-pagination';
  const previous = appendText(documentRef, pagination, 'button', '上一页', {
    type: 'button',
    'data-action': 'outlink-previous-page'
  });
  previous.className = 'batch-wizard__button batch-wizard__button--secondary';
  previous.disabled = source.loading || source.page === 0;
  appendText(
    documentRef,
    pagination,
    'span',
    `第 ${source.page + 1} / ${pageCount} 页`,
    { 'data-outlink-page-label': '' }
  );
  const next = appendText(documentRef, pagination, 'button', '下一页', {
    type: 'button',
    'data-action': 'outlink-next-page'
  });
  next.className = 'batch-wizard__button batch-wizard__button--secondary';
  next.disabled = source.loading || source.page + 1 >= pageCount;
  fragment.appendChild(pagination);

  if (draft.plan) fragment.appendChild(renderPlanPreview(documentRef, draft));
  return fragment;
}
...
function renderImportStep(documentRef, draft) {
  ...
  appendText(
    ...
    item,
    'p',
    '目标网站会先在本地检查 URL、重复项和非法站点。'
  );
  item.appendChild(renderSourceChoice(documentRef, draft));

  if (draft.sourceMode === 'outlinks') {
    item.appendChild(renderOutlinkPicker(documentRef, draft));
    return item;
  }
...
function stepIsReady(draft, step, readinessError) {
  if (draft.availabilityError) return false;
...
function availabilityFor(draft, handlers) {
  if (!draft.availabilityError) return '';
  if (typeof handlers.getAvailabilityError === 'function') {
    try {
      return text(handlers.getAvailabilityError(cloneDraft(draft)));
    } catch (_) {
      return '批次配置暂不可用，请重试加载。';
    }
  }
  return draft.availabilityError;
}
...
export function createBatchWizardView(documentRef, handlers = {}) {
  ...
  function renderCurrent(focusRequest) {
    ...
    const availabilityError = availabilityFor(draft, handlers);
    dialog.textContent = '';
    ...
    const sections = [
      renderAssignmentStep(documentRef, draft, availabilityError),
  ...
  function requestOutlinkPage(page) {
    const query = text(
      dialog.querySelector('[name="outlinkQuery"]')?.value
        ?? draft.outlinkSource.query
    );
    handlers.onOutlinkLoad?.({ query, page }, cloneDraft(draft));
  }

  function updateOutlinkSelection(selectedRecords, focus) {
    const outlinkSource = {
      ...draft.outlinkSource,
      selectedRecords: normalizeBatchOutlinkRecords(selectedRecords)
    };
    const nextDraft = {
      ...draft,
      outlinkSource,
    ...
    updateDraft(nextDraft, { focus });
    handlers.onOutlinkSelectionChange?.(
      outlinkSource.selectedRecords,
      cloneDraft(draft)
    );
  }
  ...
  function onClick(event) {
    ...
    if (action === 'retry-planning-load') {
      handlers.onRetryPlanningLoad?.(cloneDraft(draft));
      return;
    }
    if (action === 'outlink-search') {
      requestOutlinkPage(0);
      return;
    }
    if (action === 'outlink-previous-page') {
      requestOutlinkPage(Math.max(0, draft.outlinkSource.page - 1));
      return;
    }
    if (action === 'outlink-next-page') {
      requestOutlinkPage(draft.outlinkSource.page + 1);
      return;
    }
    if (action === 'outlink-select-page') {
      updateOutlinkSelection([
        ...draft.outlinkSource.selectedRecords,
        ...draft.outlinkSource.records
      ], { action: 'outlink-select-page', rowNumber: null });
      return;
    }
    if (action === 'outlink-clear-selection') {
      updateOutlinkSelection([], {
        action: 'outlink-clear-selection',
        rowNumber: null
      });
      return;
    }
  ...
  function onChange(event) {
    ...
    if (input.name === 'sourceMode') {
      const sourceMode = input.value === 'outlinks' ? 'outlinks' : 'csv';
      const nextDraft = {
        ...draft,
        sourceMode,
        preflight: null,
      ...
      updateDraft(nextDraft, { focus: { name: 'sourceMode' } });
      handlers.onSourceModeChange?.(sourceMode, cloneDraft(draft));
      return;
    }
    if (input.name === 'outlinkRecordSelection') {
      const selectedByUrl = new Map(
        draft.outlinkSource.selectedRecords.map((record) => [record.url, record])
      );
      const record = draft.outlinkSource.records.find(
        (candidate) => candidate.url === input.value
      );
      if (!record) return;
      if (input.checked) selectedByUrl.set(record.url, record);
      else selectedByUrl.delete(record.url);
      updateOutlinkSelection([...selectedByUrl.values()], {
        name: 'outlinkRecordSelection'
      });
      return;
    }
    const mappingNames = {
    ...
    if (
      ...
      const selectedProfileIds = [
        ...dialog.querySelectorAll('input[name="selectedProfileIds"]:checked')
      ]
        .map(({ value }) => value);
      const selectedPromotionPageIds = [
        ...dialog.querySelectorAll('input[name="selectedPromotionPageIds"]:checked')
      ]
        .map(({ value }) => value);
      draft = normalizeDraft({
        ...draft,
      ...
      emitDraftChange();
      handlers.onAllocationSelectionChange?.({
        selectedProfileIds: draft.selectedProfileIds,
        selectedPromotionPageIds: draft.selectedPromotionPageIds
      }, cloneDraft(draft));
      return;
    }
  ...
  function onKeydown(event) {
    if (!opened) return;
    if (event.key === 'Enter' && event.target?.name === 'outlinkQuery') {
