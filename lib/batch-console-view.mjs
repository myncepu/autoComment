...
function rowSummary(row) {
  if (row?.status === 'skipped') {
    return batchSkipReasonLabel(row?.skipReason);
  }
...
function renderCommandActions(documentRef, parent, snapshot) {
  ...
  if (command.canResume || snapshot?.status === 'paused_recovery') {
  ...
  button(documentRef, parent, '导出诊断日志', 'export-diagnostics', {
    disabled: busy || !snapshot?.batchId
  });
...
function renderFallbackTable(documentRef, snapshot, rows) {
  const wrapper = documentRef.createElement('div');
...
function renderDataGridMount(documentRef) {
  const wrapper = documentRef.createElement('div');
  wrapper.className = 'batch-console__table-wrap batch-console__table-wrap--grid';
  wrapper.setAttribute('aria-label', '可排序、可筛选的完整目标队列');
  const mount = documentRef.createElement('div');
  mount.className = 'batch-console__data-grid';
  mount.dataset.taskGrid = '';
  wrapper.appendChild(mount);
  return wrapper;
}

function dataGridTextFormatter(documentRef, { preview = false } = {}) {
  return (cell) => {
    const value = text(cell.getValue(), '—');
    const span = documentRef.createElement('span');
    span.textContent = value || '—';
    if (preview) {
      span.className = 'batch-console__preview-value';
      span.dataset.previewValue = '';
      span.title = value;
      span.tabIndex = 0;
    }
    return span;
...
function dataGridRows(snapshot, rows) {
  return rows.map((row) => ({
    ...row,
    gridBusy: Boolean(commandState(snapshot).inFlight),
    gridStatus: rowStatusLabel(row),
    gridElapsed: formatElapsed(row?.elapsedMs),
    gridSummary: rowSummary(row),
    gridTab: tabLabel(snapshot, row),
    gridAnchors: Array.isArray(row?.anchorTexts)
      ? row.anchorTexts.join(' · ')
      : '',
    gridSubmittedAt: Number.isFinite(row?.submittedAt)
      ? row.submittedAt
      : null
  }));
}

function dataGridColumns(documentRef) {
  const plain = dataGridTextFormatter(documentRef);
  const preview = dataGridTextFormatter(documentRef, { preview: true });
  const textColumn = (title, field, width, extra = {}) => ({
    title,
    field,
    sorter: 'string',
    headerFilter: 'input',
  ...
  const statusColumn = {
    ...
    formatter: (cell) => {
      const span = documentRef.createElement('span');
      span.textContent = ROW_STATUS_LABELS[cell.getValue()] ||
        text(cell.getValue(), '未知');
      return span;
    },
    minWidth: 130
  };
  const submittedAtColumn = {
    title: '提交成功时间',
    field: 'gridSubmittedAt',
    sorter: 'number',
    headerFilter: 'input',
    headerFilterPlaceholder: '筛选日期/时间…',
    headerFilterFunc: (headerValue, rowValue) => (
      !text(headerValue) ||
      (
        Number.isFinite(rowValue) &&
        formatTimestamp(rowValue).includes(text(headerValue))
      )
    ),
    formatter: (cell) => {
      const span = documentRef.createElement('span');
  ...
  return [
    {
      ...
      headerFilter: 'input',
      headerFilterFunc: '=',
      headerFilterPlaceholder: '序号',
      hozAlign: 'right',
      width: 82
    },
    textColumn('目标 URL', 'url', 240),
    statusColumn,
    submittedAtColumn,
    textColumn('身份', 'profileLabel', 140),
    textColumn('推广网站', 'promotionSiteLabel', 150),
    textColumn('评论文本', 'commentText', 190, { formatter: preview }),
    textColumn('锚文本', 'gridAnchors', 150, { formatter: preview }),
    textColumn('推广网址', 'promotedWebsiteUrl', 180, {
      formatter: preview
    }),
    {
      title: '耗时',
      ...
      formatter: (cell) => {
        const span = documentRef.createElement('span');
        span.textContent = formatElapsed(cell.getValue());
        return span;
      },
      minWidth: 100
    },
    textColumn('阶段或错误', 'gridSummary', 210),
    textColumn('worker 标签页', 'gridTab', 130),
    {
      title: '操作',
      field: 'taskId',
      headerSort: false,
      formatter: (cell) => {
        ...
        renderRowActions(
          ...
          group,
          cell.getRow().getData(),
          cell.getRow().getData()?.gridBusy === true
        );
...
function renderQueue(
  ...
  section.append(
    useDataGrid
      ? renderDataGridMount(documentRef)
      : renderFallbackTable(documentRef, snapshot, rows),
...
export function createBatchConsoleView(
  documentRef,
  handlers = {},
  options = {}
) {
  ...
  const TabulatorConstructor = options.Tabulator ||
    documentRef.defaultView?.Tabulator ||
    globalThis.Tabulator;
  const useDataGrid = typeof TabulatorConstructor === 'function';
  ...
  let destroyed = false;
  let dataGrid = null;
  let dataGridRefreshFailed = false;
  let dataGridRenderedRows = [];
  const dataGridState = {
    sorters: [],
    headerFilters: [],
    page: 1,
  ...
  function captureDataGridState() {
    if (!dataGrid) return;
    try {
      dataGridState.sorters = dataGrid.getSorters()
        .map((sorter) => ({
          column: sorter.field || sorter.column?.getField?.(),
          dir: sorter.dir
        }))
        .filter((sorter) => text(sorter.column));
      dataGridState.headerFilters = dataGrid.getHeaderFilters()
        .map((filter) => ({
          field: filter.field,
          value: filter.value
        }));
      dataGridState.page = Math.max(1, integer(dataGrid.getPage?.(), 1));
    } catch (_) {}

    const active = documentRef.activeElement;
    const header = active?.closest?.('.tabulator-col[tabulator-field]');
    if (!header?.contains(active)) {
      dataGridState.focusedHeaderField = '';
      return;
    }
    dataGridState.focusedHeaderField =
      header.getAttribute('tabulator-field') || '';
    dataGridState.selectionStart = Number.isInteger(active.selectionStart)
      ? active.selectionStart
      : null;
    dataGridState.selectionEnd = Number.isInteger(active.selectionEnd)
      ? active.selectionEnd
      : null;
  }
  ...
  function destroyDataGrid() {
    if (!dataGrid) return;
    captureDataGridState();
    try {
      dataGrid.destroy();
    } catch (_) {}
    dataGrid = null;
    dataGridRenderedRows = [];
  }

  function restoreDataGridFocus() {
    const field = dataGridState.focusedHeaderField;
    if (!field || !dataGrid) return;
    try {
      dataGrid.setHeaderFilterFocus(field);
    } catch (_) {}
    const input = [...root.querySelectorAll(
      '.tabulator-col[tabulator-field] .tabulator-header-filter input'
    )].find((candidate) => (
      candidate.closest('.tabulator-col')
        ?.getAttribute('tabulator-field') === field
    ));
    if (!input) return;
    input.focus();
    if (
      dataGridState.selectionStart !== null &&
      typeof input.setSelectionRange === 'function'
    ) {
      input.setSelectionRange(
        dataGridState.selectionStart,
        dataGridState.selectionEnd
      );
    }
  }

  function mountDataGrid() {
    if (!useDataGrid || !snapshot) return;
    const mount = root.querySelector('[data-task-grid]');
    if (!mount) return;
    const rows = visibleRows(snapshot);
    try {
      const initialRows = dataGridRows(snapshot, rows);
      const instance = new TabulatorConstructor(mount, {
        data: initialRows,
        index: 'taskId',
        columns: dataGridColumns(documentRef),
        layout: 'fitDataStretch',
        maxHeight: '68vh',
        placeholder: '没有符合当前筛选条件的目标',
        pagination: true,
        paginationSize: 50,
        paginationSizeSelector: [25, 50, 100, 200],
        paginationCounter: 'rows',
        headerFilterLiveFilterDelay: 250,
        initialSort: dataGridState.sorters,
        initialHeaderFilter: dataGridState.headerFilters,
        columnDefaults: {
        ...
        rowFormatter(rowComponent) {
          ...
          element.dataset.taskRow = String(integer(row?.urlIndex));
          element.dataset.attempt = String(
            Math.max(1, integer(row?.attempt, 1))
          );
          element.setAttribute('aria-label', rowLabel(row));
        }
      });
      dataGrid = instance;
      dataGridRenderedRows = initialRows;
      dataGridRefreshFailed = false;
      instance.on('tableBuilt', () => {
        if (dataGrid !== instance) return;
        if (dataGridState.page > 1) {
          Promise.resolve(instance.setPage(dataGridState.page)).catch(() => {});
        }
        restoreDataGridFocus();
      });
    } catch (_) {
      dataGrid = null;
      mount.closest('.batch-console__table-wrap')?.replaceWith(
        renderFallbackTable(documentRef, snapshot, rows)
      );
    }
  }
  ...
  function refreshDataGridInPlace() {
    if (
      !useDataGrid ||
      !dataGrid ||
      dataGridRefreshFailed ||
      !snapshot?.batchId ||
      snapshot.status === 'empty' ||
      visibleRows(snapshot).length === 0
    ) {
      return false;
    }
    const layout = shell?.content?.querySelector('.batch-console__layout');
    const overview = layout?.querySelector('[data-console-overview]');
    const queue = layout?.querySelector('.batch-console__queue');
    const toolbar = queue?.querySelector('.batch-console__toolbar');
    const cards = queue?.querySelector('[data-task-cards]');
    const mount = queue?.querySelector('[data-task-grid]');
    if (!layout || !overview || !queue || !toolbar || !cards || !mount) {
      return false;
    }

    overview.replaceWith(renderOverview(documentRef, snapshot));
    renderToolbar(documentRef, snapshot, toolbar);
    cards.replaceWith(
      renderCards(documentRef, snapshot, visibleRows(snapshot))
    );

    const instance = dataGrid;
    const nextRows = dataGridRows(snapshot, visibleRows(snapshot));
    const sameRowIdentity = (
      dataGridRenderedRows.length === nextRows.length &&
      dataGridRenderedRows.every((row, index) => (
        row.taskId === nextRows[index]?.taskId
      ))
    );
    const changedRows = sameRowIdentity
      ? nextRows.filter((row, index) => (
          JSON.stringify(row) !== JSON.stringify(dataGridRenderedRows[index])
        ))
      : nextRows;
    dataGridRenderedRows = nextRows;
    try {
      const update = sameRowIdentity
        ? (
            changedRows.length > 0
              ? instance.updateData(changedRows)
              : null
          )
        : instance.replaceData(nextRows);
      Promise.resolve(update).catch(() => {
        if (dataGrid === instance) dataGridRefreshFailed = true;
      });
      return true;
    } catch (_) {
      dataGridRefreshFailed = true;
      return false;
    }
  }
  ...
  function renderCurrent() {
    ...
    if (!snapshot.batchId || snapshot.status === 'empty') {
      ...
      layout.append(
        ...
        renderQueue(documentRef, snapshot, toolbar, { useDataGrid })
      );
