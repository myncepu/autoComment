...
function renderOverview(documentRef, snapshot) {
  const overview = documentRef.createElement('aside');
  ...
  overview.append(
    ...
    renderSlots(documentRef, snapshot),
...
function renderToolbar(documentRef, snapshot) {
  ...
  selectField(documentRef, toolbar, {
    ...
    options: [['all', '全部域名'], ...domains.map((domain) => [domain, domain])]
  });
...
function renderTable(documentRef, snapshot, rows) {
  ...
  for (const label of ['序号', '目标 URL', '状态', '耗时', '阶段或错误', 'worker 标签页', '操作']) {
...
function renderQueue(documentRef, snapshot) {
  const section = documentRef.createElement('section');
  ...
  section.appendChild(renderToolbar(documentRef, snapshot));
...
export function createBatchConsoleView(documentRef, handlers = {}) {
  ...
  function renderCurrent() {
    ...
    root.append(
      renderCommandBar(documentRef, snapshot),
      renderBanners(documentRef, snapshot),
      renderSummary(documentRef, snapshot)
    );

    if (!snapshot.batchId || snapshot.status === 'empty') {
      root.appendChild(renderEmpty(documentRef));
    } else {
      const layout = documentRef.createElement('div');
      layout.className = 'batch-console__layout';
      layout.append(
        renderOverview(documentRef, snapshot),
        renderQueue(documentRef, snapshot)
      );
  ...
  function onKeydown(event) {
    ...
    if (event.key === 'Escape') {
      ...
      if (!commandState(snapshot).inFlight) closeLayer();
      return;
    }
