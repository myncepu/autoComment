export const HISTORY_MESSAGE_TYPES = Object.freeze({
  ...
  LIST: 'HISTORY_LIST',
  RECENT_SUCCESS_URLS: 'HISTORY_RECENT_SUCCESS_URLS',
...
function normalizeFilter(filter = {}) {
  const normalized = {
    ...
    profileId: normalizedId(filter.profileId),
    promotionSiteId: normalizedId(filter.promotionSiteId),
...
function exportFilterFromMessage(message) {
  const { exportedBefore: _ignored, ...filter } = filterFromMessage(message);
  return filter;
}
...
function routeHistoryMessage(message, service) {
  switch (message.type) {
    ...
    case HISTORY_MESSAGE_TYPES.RECENT_SUCCESS_URLS:
      if (finiteNumber(message.since) === undefined) {
        throw new TypeError('A finite recent-success cutoff is required');
      }
      return service.listRecentSuccessfulTargetUrls({
        since: message.since
      });
    ...
    case HISTORY_MESSAGE_TYPES.EXPORT_START:
      return service.startExport(exportFilterFromMessage(message));
