...
export function createBatchRuntimeController({
  ...
  async function proofBoundTaskHook(
    ...
    const pendingSideEffect = Promise.resolve().then(() => sideEffectHook({
      checkpoint: structuredClone(checkpoint),
    ...
    if (outcome.kind === 'timeout') {
      return {
        ok: false,
        error: 'proof_side_effect_timeout',
        checkpoint,
        recoveryRequired: true
      };
    }
    if (outcome.kind === 'rejected') {
      return {
        ok: false,
        error: safeError(outcome.error, 'proof_side_effect_failed'),
        checkpoint,
        recoveryRequired: true
      };
    }
    const sideEffect = outcome.value;
    return {
      ok: true,
      checkpoint,
      changed: false,
  ...
  function runOwnerPageRecoveryHook(
    message,
    sender,
    targetTabId,
    sideEffectHook
  ...
  async function terminalTask(message, sender, result, sideEffectHook) {
    ...
    if (task.state === 'queued') {
      ...
      if (reservation) {
        ...
        checkpoint = clearOpening(checkpoint, requestId);
    ...
    } else if (openingRequestId) {
      try {
        await sessionJournal.remove(openingRequestId);
      } catch (_) {
        // The durable opening token is already cleared.
      }
    }
    return {
      ok: true,
        ...
        void creation.then(
          async (lateTab) => {
            if (
              ...
              lateTab.windowId !== windowId
            ) {
              return;
            }
            await tabs.remove(lateTab.id).catch(() => {
              logger.warn?.(
                '[batch-runtime] Failed to close a late-created pending tab'
              );
            });
          },
          () => {}
