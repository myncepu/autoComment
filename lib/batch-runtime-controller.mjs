...
export function createBatchRuntimeController({
  ...
  async function removeTaskWithProof(checkpoint, task) {
    ...
    try {
      ...
      return isMissingTabError(error)
        ? { ok: true, missing: true, requestId: identity.requestId }
        : {
            ok: false,
    ...
    try {
      await tabs.remove(identity.tabId);
      return { ok: true, missing: false, requestId: identity.requestId };
    } catch (error) {
      return isMissingTabError(error)
        ? { ok: true, missing: true, requestId: identity.requestId }
        : {
  ...
  async function terminalTask(message, sender, result) {
    ...
    if (ownsTab) {
      ...
      if (!internal && !contentOwnsTask && !pageOwnsTask) {
        return {
          ok: false,
      ...
      if (!removal.ok) {
        const recovery = pausedOwnershipCheckpoint(
          checkpoint,
          removal.reason === 'ownership_unverified'
        ...
        return {
          ok: false,
          error: removal.reason === 'ownership_unverified'
            ? 'batch_ownership_unverified'
            : 'batch_teardown_cleanup_failed',
          checkpoint: recovery,
          recoveryRequired: true
  ...
  function markTerminal(message, sender) {
