...
export function createBatchRuntimeController({
  ...
  function observeOpeningCleanup(checkpoint, requestId, observedAt) {
    const next = structuredClone(checkpoint);
    const reservation = openingReservations(next)[requestId];
    if (!reservation?.cleanupOnly) return next;
    reservation.cleanupObservedAt = observedAt;
  ...
  async function teardownPage(message) {
    ...
    if (
      ...
      for (const reservation of reservations) {
        ...
        if (exactTabs.length === 0) {
          ...
          } else if (reservation.cleanupOnly) {
            const observedAt = now();
            const pendingInThisController =
              pendingCreateRequestIds.has(identity.requestId);
            const quiescenceStarted =
              Number.isFinite(reservation.cleanupObservedAt);
            if (
              pendingInThisController ||
              !quiescenceStarted ||
              observedAt - reservation.cleanupObservedAt <
                Math.max(1, lateCreateQuiescenceMs)
            ) {
              if (!pendingInThisController && !quiescenceStarted) {
              ...
              failures.push({
                reason: 'tab_close_failed',
                diagnostic: pendingInThisController
                  ? 'late_create_still_pending'
                  : 'late_create_quiescing'
              });
            }
          }
          continue;
    ...
    if (failures.length > 0) {
      let recoveryBase = checkpoint;
      for (const observation of cleanupObservations) {
        recoveryBase = observeOpeningCleanup(
          recoveryBase,
