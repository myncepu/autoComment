...
export function createBatchRuntimeController({
  ...
  recoverRemovedSubmitContext = null,
  ...
  function savedPageOwnership(checkpoint) {
    const ownership = checkpoint?.batchPageOwnership;
    if (
      ownership?.batchId !== checkpoint?.batchId ||
      !Number.isInteger(ownership?.tabId) ||
      ownership.tabId <= 0 ||
      typeof ownership?.documentId !== 'string' ||
      ownership.documentId.length === 0
    ) {
      return null;
    }
    return ownership;
  ...
  function pageOwnershipPayload(checkpoint) {
    return { batchId: checkpoint.batchId };
  }

  function bindPageOwnership(checkpoint, sender, { force = false } = {}) {
    if (
      !checkpoint ||
    ...
    const existing = savedPageOwnership(checkpoint);
    const resourceOwnerTabIds = new Set([
      ...Object.values(checkpoint.tasks || {})
        .filter((task) => ['active', 'submitting'].includes(task?.state))
        .map((task) => task.ownerPageTabId),
      ...Object.values(openingReservations(checkpoint))
        .map((reservation) => reservation.ownerPageTabId)
    ...
    if (
      ...
      (
        ...
        (
          ...
          resourceOwnerTabIds.size > 0 &&
          (
            resourceOwnerTabIds.size !== 1 ||
            !resourceOwnerTabIds.has(sender.tab.id)
          )
        )
      )
    ) {
    ...
    if (
      existing?.tabId === sender.tab.id &&
      existing.documentId === sender.documentId
    ) {
      return {
        checkpoint,
        claimed: true,
  ...
  function ownsPageTeardown(checkpoint, sender) {
    if (!isBatchPageSender(sender, runtime)) return false;
    const ownership = savedPageOwnership(checkpoint);
    if (ownership) {
      return ownership.tabId === sender.tab.id &&
        ownership.documentId === sender.documentId;
    }
    const resources = [
      ...Object.values(checkpoint.tasks || {}).filter(
        (task) => ['active', 'submitting'].includes(task?.state)
      ),
      ...Object.values(openingReservations(checkpoint))
    ];
    return resources.length > 0 && resources.every(
      (resource) => resource.ownerPageTabId === sender.tab.id
    );
  }
  ...
  async function hasLiveBatchPageOwner(checkpoint) {
    const ownership = savedPageOwnership(checkpoint);
    const ownerTabIds = new Set([
      ...(ownership?.released === true ? [] : [ownership?.tabId]),
      ...Object.values(checkpoint?.tasks || {})
        .filter((task) => ['active', 'submitting'].includes(task?.state))
        .map((task) => task.ownerPageTabId),
      ...Object.values(openingReservations(checkpoint))
    ...
    const expectedUrl = runtime.getURL('batch.html');
    for (const tabId of ownerTabIds) {
      let tab;
      try {
        tab = await boundedTabGet(tabId);
      } catch (error) {
        if (isMissingTabError(error)) continue;
        return true;
      }
      ...
      try {
        ...
        // A live tab outside the batch page no longer owns this console.
      }
    }
    return false;
  }
  ...
  function finalizedCheckpoint(checkpoint, reason, finalStatus = null) {
    ...
    next.openingReservations = {};
    if (savedPageOwnership(next)) {
      next.batchPageOwnership.released = true;
    }
    if (finalStatus !== null) {
      next.status = finalStatus;
    } else if (['terminated', 'completed'].includes(sourceStatus)) {
  ...
  async function teardownPage(
    ...
    if (
      ...
      (
        ...
        !ownsPageTeardown(loaded.checkpoint, sender)
      )
    ) {
      return {
        ok: false,
        error: 'stale_batch_page',
        checkpoint: loaded.checkpoint,
        cleanupComplete: false
      };
    }
    ...
    const finalized = finalizedCheckpoint(checkpoint, reason, finalStatus);
  ...
  async function start(message, sender) {
    ...
    if (message.plan) {
      ...
      const checksLegacyRecentHistory = message.plan.tasks.some((task) => (
        task?.state === 'eligible' && task.assignmentSource !== 'round_robin'
      ));
      if (checksLegacyRecentHistory) {
        ...
        const newlyRecent = message.plan.tasks.some((task) => (
          task?.state === 'eligible'
          && task.assignmentSource !== 'round_robin'
      ...
      try {
        const stats = await loadSuccessfulTargetStats();
        if (!Array.isArray(stats)) throw new Error();
        successfulTargets = successStatsMap(stats);
      } catch (_) {
        return { ok: false, error: 'outlink_success_history_unavailable' };
      }
      const repeatedPromotion = message.plan.tasks.some((task) => {
        if (task?.state !== 'eligible') return false;
        const site = message.plan.promotionSites?.[task.promotionSiteId];
        return exactPromotionAlreadySucceeded(
          successfulTargets.get(canonicalSuccessHost(task.targetDomain)),
          task.canonicalTargetUrl,
          site?.url
        );
      });
      if (repeatedPromotion) {
        return {
          ok: false,
    ...
    const pageBinding = bindPageOwnership(
      checkpoint,
      sender,
      { force: true }
    );
    checkpoint = pageBinding.checkpoint;
    ...
    return {
      ...
      ...(pageBinding.claimed
        ? {
            pageOwnership: pageOwnershipPayload(
  ...
  async function stopSession(message) {
    ...
    const stopped = await teardownPage({
  ...
  async function loadForPageRequest(sender) {
    const loaded = await readCheckpoint();
    if (!loaded.ok || !loaded.checkpoint) return loaded;
    if (sender != null) {
      const claim = bindPageOwnership(loaded.checkpoint, sender);
      if (!claim.claimed) {
        if (!await hasLiveBatchPageOwner(loaded.checkpoint)) {
          const recovered = await normalizeForRecovery();
          if (!recovered.ok || !recovered.checkpoint) return recovered;
          const rebound = bindPageOwnership(
            recovered.checkpoint,
            sender,
            { force: true }
          );
          try {
            await writeCheckpoint(rebound.checkpoint);
          } catch (_) {
            return {
              ok: false,
              error: 'checkpoint_write_failed',
          ...
          return {
            ...
            pageOwnership: pageOwnershipPayload(rebound.checkpoint)
          };
        }
        return {
          ok: false,
          error: 'batch_page_owned_elsewhere',
          checkpoint: loaded.checkpoint,
          recoveryRequired: hasDurableOwnership(loaded.checkpoint)
        };
      }
      if (claim.changed) {
        try {
          await writeCheckpoint(claim.checkpoint);
        } catch (_) {
          return {
            ok: false,
            error: 'checkpoint_write_failed',
    ...
    const response = await normalizeForRecovery();
    if (!response.ok || !response.checkpoint) return response;
    const binding = bindPageOwnership(response.checkpoint, sender);
    if (binding.changed) {
      try {
        await writeCheckpoint(binding.checkpoint);
      } catch (_) {
        return {
          ok: false,
          error: 'checkpoint_write_failed',
  ...
  function expireTask(message) {
    return enqueue(async () => {
      ...
      if (
        ...
        return {
          ok: false,
          error: checkpoint.batchId === message?.batchId
            ? 'stale_attempt'
            : 'stale_batch',
          checkpoint
        };
      ...
      const uncertain = task.state === 'submitting' ||
        ['submitting', 'confirming', 'closing'].includes(task.phase);
      const tabId = task.tabId;
      const response = await terminalTask(message, null, {
        result: uncertain ? 'manual_required' : 'fail',
        aiContent: null,
        errorCode: uncertain ? 'submission_uncertain' : 'task_timeout',
  ...
  function handleWorkerTabRemoved(tabId) {
    return enqueue(async () => {
      ...
      const ownerPageRemoved = [
        ...Object.values(checkpoint.tasks || {}).filter(
          (task) => ['active', 'submitting'].includes(task?.state)
        ),
        ...Object.values(openingReservations(checkpoint))
      ].some((resource) => resource.ownerPageTabId === tabId);
      if (ownerPageRemoved) {
        return teardownPage({
          batchId: checkpoint.batchId,
          reason: 'owner_page_closed'
        });
      }
      ...
      let submitContextRecovery = null;
      if (typeof recoverRemovedSubmitContext === 'function') {
        try {
          submitContextRecovery = await recoverRemovedSubmitContext({
            tabId,
            expected: {
              batchId: checkpoint.batchId,
              taskId: task.taskId,
          ...
          return {
            ok: false,
            error: 'submit_context_recovery_failed',
            checkpoint,
            recoveryRequired: true
          };
        }
      }
      const submitUncertain =
        submitContextRecovery?.recovered === true ||
        submitContextRecovery?.sealed === false;
      const candidate = applyBatchRuntimeEvent(checkpoint, {
        type: 'task_terminal',
        batchId: checkpoint.batchId,
        taskId: task.taskId,
        ...
        result: createWorkerTabRemovalResult(
          submitUncertain
        ...
        switch (message?.type) {
          ...
          case 'BATCH_PAGE_TEARDOWN':
            return await teardownPage(message, {
              sender,
              requirePageOwnership: sender != null
            });
