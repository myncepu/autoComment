...
export function createBatchRuntimeController({
  ...
  function discoverPendingWorkers(checkpoint, openTabs) {
  ...
  function discoverOwnedTaskTabs(checkpoint, openTabs) {
    return openTabs.flatMap((tab) => {
      if (!Number.isInteger(tab?.id) || !Number.isInteger(tab?.windowId)) {
        return [];
      }
      const task = Object.values(checkpoint.tasks).find((candidate) => (
        ['active', 'submitting'].includes(candidate.state) &&
        candidate.tabId === tab.id &&
        candidate.windowId === tab.windowId
      ));
      if (!task) return [];
      const item = checkpoint.source?.parsedUrls?.[task.urlIndex];
      const liveUrl = tab.pendingUrl || tab.url;
      if (
        typeof item?.url !== 'string' ||
        (
          liveUrl !== item.url &&
          liveUrl !== pendingWorkerUrl(task.requestId)
        )
      ) {
        return [];
      }
  ...
  function discoverExtraOwnedTab(checkpoint, openTabs, ownership) {
    if (
      ownership?.batchId !== checkpoint.batchId ||
      !Number.isInteger(ownership.urlIndex) ||
      !Number.isInteger(ownership.attempt) ||
      ownership.attempt < 1 ||
      !Number.isInteger(ownership.tabId) ||
      ownership.tabId < 1 ||
      !Number.isInteger(ownership.windowId) ||
      ownership.windowId < 1
    ) {
      return [];
    }
    const task = checkpoint.tasks[String(ownership.urlIndex)];
    const item = checkpoint.source?.parsedUrls?.[ownership.urlIndex];
    const requestId = `${checkpoint.batchId}:${ownership.urlIndex}:` +
      ownership.attempt;
    const tab = openTabs.find((candidate) => (
      candidate?.id === ownership.tabId &&
      candidate?.windowId === ownership.windowId
    ));
    const liveUrl = tab?.pendingUrl || tab?.url;
    if (
      task?.attempt !== ownership.attempt ||
      typeof item?.url !== 'string' ||
      (liveUrl !== item.url && liveUrl !== pendingWorkerUrl(requestId))
    ) {
      return [];
    }
  ...
  function retainFailedTaskOwnership(checkpoint, source, failedTabIds) {
    const failed = new Set(failedTabIds);
    const next = structuredClone(checkpoint);
    for (const task of Object.values(source.tasks)) {
      if (
        ['active', 'submitting'].includes(task.state) &&
        failed.has(task.tabId)
      ) {
        next.tasks[String(task.urlIndex)] = structuredClone(task);
      }
    }
    return next;
  }
  ...
  async function teardownPage(message, extraOwnership = null) {
    ...
    try {
      ...
      discoveredPending = discoverPendingWorkers(
        loaded.checkpoint,
        openTabs
      );
      ownedTaskTabIds = discoverOwnedTaskTabs(
        loaded.checkpoint,
        openTabs
      );
      ownedTaskTabIds.push(
        ...discoverExtraOwnedTab(
          loaded.checkpoint,
          openTabs,
          extraOwnership
        )
