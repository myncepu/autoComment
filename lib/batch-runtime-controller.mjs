...
export function createBatchRuntimeController({
  ...
  function reservationTabIds(checkpoint) {
    return Object.values(openingReservations(checkpoint))
      .map((reservation) => reservation?.tabId)
  ...
  function resetMissingActive(checkpoint, {
    ...
    delete next.tasks[String(urlIndex)].requestId;
  ...
  async function teardownPage(message, extraOrphanTabIds = []) {
    ...
    const orphanTabIds = [
      ...new Set([
        ...
        ...reservationTabIds(loaded.checkpoint),
        ...extraOrphanTabIds
      ])
    ].filter(Number.isInteger);
  ...
  async function normalizeForRecovery() {
    ...
    const retryOrphans =
      loaded.checkpoint.recoveryCleanup?.orphanTabIds || [];
    const pendingOpeningTabs = reservationTabIds(loaded.checkpoint);
    if (
      loaded.checkpoint.status !== 'running' &&
      retryOrphans.length === 0 &&
      pendingOpeningTabs.length === 0
    ) {
      return {
        ok: true,
        checkpoint: loaded.checkpoint,
  ...
  async function createWorkerTab(message, sender) {
    ...
    if (task.state === 'active') {
      ...
      let existingTab;
      try {
        existingTab = await tabs.get(task.tabId);
      } catch (error) {
