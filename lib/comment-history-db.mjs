...
import { isDomainEntityType } from './cloud-sync-domain-config.mjs';
import { summarizeSuccessfulComments } from './outlink-success-stats.mjs';
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!database.objectStoreNames.contains(stores.comments)) {
        comments = database.createObjectStore(stores.comments, { keyPath: 'id' });
        ...
        comments = request.transaction.objectStore(stores.comments);
      }
      if (!comments.indexNames.contains('by_profile_submitted_at')) {
        comments.createIndex(
          'by_profile_submitted_at',
          ['profileId', 'submittedAt', 'id']
        );
      }
      if (!comments.indexNames.contains('by_promotion_site_submitted_at')) {
        comments.createIndex(
          'by_promotion_site_submitted_at',
          ['promotionSiteId', 'submittedAt', 'id']
        );
      }
...
function normalizedFilter(filter = {}) {
  return {
    ...
    promotionSiteId: filter.promotionSiteId,
...
function matchesComment(comment, filter) {
  ...
  if (filter.profileId != null && comment.profileId !== filter.profileId) return false;
  if (filter.promotionSiteId != null && comment.promotionSiteId !== filter.promotionSiteId) return false;
...
export async function openCommentHistoryDb({
  ...
  async function initializeBootstrapSentinel({
    vaultId,
    state,
    protocolVersion = 1
  }) {
    ...
    const stateKey = protocolVersion === 2
      ? `bootstrapState:v2:${validatedVaultId}`
      : `bootstrapState:${validatedVaultId}`;
    const transaction = database.transaction(stores.syncMeta, 'readwrite');
    const completion = transactionCompletion(transaction);
    const meta = transaction.objectStore(stores.syncMeta);
    try {
      const existing = await requestResult(meta.get(stateKey));
      if (!existing) meta.put({ key: stateKey, value: state });
      meta.put({
        key: `authBlocked:${validatedVaultId}`,
        value: null
      });
      await completion;
  ...
  async function applyRemoteChangeInTransaction(transaction, vaultId, change) {
    ...
    if (isDomainEntityType(change.entityType)) {
      try {
        normalizeSyncMutation({
      ...
      entities.put({
        entityKey: `${vaultId}:${change.entityType}:${change.entityId}`,
        vaultId,
        revisionId: null,
        serverSeq: change.serverSeq
      });
  ...
  async function applyRemoteChangesAtomic({
    ...
    try {
      const meta = transaction.objectStore(stores.syncMeta);
      const cursorKey = protocolVersion === 2
        ? `serverCursor:v2:${validatedVaultId}`
        : `serverCursor:${validatedVaultId}`;
  ...
  async function applyBootstrapPageAtomic({
    ...
    if (
      ...
      || domainChanges.length > 100
      || (protocolVersion !== 1 && protocolVersion !== 2)
      || (protocolVersion === 1 && domainChanges.length > 0)
    ...
    if (protocolVersion === 2) {
      for (const change of domainChanges) {
        ...
        try {
          normalizeSyncMutation({
            mutationId: 'bootstrap-domain-change',
            entityType: change.entityType,
            entityId: change.entityId,
            operation: change.operation,
            payload: change.payload,
    ...
    try {
      ...
      const incrementalCursorRecord = await requestResult(
        meta.get(protocolVersion === 2
          ? `serverCursor:v2:${validatedVaultId}`
          : `serverCursor:${validatedVaultId}`)
      );
      const incrementalCursor = incrementalCursorRecord?.value;
      if (
        incrementalCursor !== undefined
        && (
          !Number.isSafeInteger(incrementalCursor)
          || incrementalCursor < 0
          || serverCursor < incrementalCursor
        )
      ) {
        throw historyDbError(
          'SYNC_CURSOR_REGRESSION',
      ...
      meta.put({
        ...
        value: {
          ...
          ...(protocolVersion === 2
            ? {
                protocolVersion: 2,
                domainChanges: hasMore
                  ? [
                      ...(Array.isArray(savedState?.domainChanges)
                        ? savedState.domainChanges
                        : []),
                      ...structuredClone(domainChanges)
                    ]
                  : []
              }
            : {})
  ...
  function normalCursorPlan(commentStore, filter, cursor) {
    ...
    } else if (filter.profileId != null) {
      indexName = 'by_profile_submitted_at';
      prefix = [filter.profileId];
    } else if (filter.promotionSiteId != null) {
      indexName = 'by_promotion_site_submitted_at';
      prefix = [filter.promotionSiteId];
    }
  ...
  async function listRecentSuccessfulTargetUrls({ since } = {}) {
    if (!Number.isFinite(since) || !IDBKeyRangeImpl) {
      throw new TypeError('A finite since value and IDBKeyRange are required');
    }
    const transaction = database.transaction(stores.comments, 'readonly');
    const completion = transactionCompletion(transaction);
    const index = transaction.objectStore(stores.comments).index('by_submitted_at');
    const urls = [];
    const seen = new Set();
    await cursorAll(
      index,
      IDBKeyRangeImpl.lowerBound(since),
      'prev',
      (cursor, next) => {
        const url = cursor.value?.targetPageUrl;
  ...
  async function listSuccessfulTargetStats() {
    const transaction = database.transaction(stores.comments, 'readonly');
    const completion = transactionCompletion(transaction);
    const comments = await collectCursor(
      transaction.objectStore(stores.comments),
      null,
      'next'
    );
    await completion;
    return summarizeSuccessfulComments(comments);
  }
