import { normalizeCommentRevision } from './cloud-sync-protocol.mjs';
...
export async function openCommentHistoryDb({
  ...
  async function upsertIfFresher({ comment, anchors }, { syncMutation } = {}) {
    const transaction = database.transaction(
      syncMutation
        ? [stores.comments, stores.anchors, stores.syncOutbox]
  ...
  async function applyRemoteChangeInTransaction(transaction, vaultId, change) {
    if (!change || typeof change !== 'object' || !Number.isInteger(change.serverSeq)) {
  ...
  async function applyBootstrapPageAtomic({
    ...
    try {
      ...
      meta.put({
        key: `bootstrapState:${validatedVaultId}`,
        value: {
          cursor: nextCursor,
          serverCursor,
          done: !hasMore
