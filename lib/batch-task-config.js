((root) => {
  ...
  const MODERN_PROMOTION_SITE_KEYS = [...PROMOTION_SITE_KEYS, 'email'];
  ...
  function validPromotionSite(site) {
    return (hasExactKeys(site, PROMOTION_SITE_KEYS)
        || hasExactKeys(site, MODERN_PROMOTION_SITE_KEYS))
      && PROMOTION_SITE_KEYS.every((key) => typeof site[key] === 'string')
      && (!Object.hasOwn(site, 'email') || typeof site.email === 'string')
      && identifier(site.id)
  ...
  function validateHandle(message) {
    ...
    if (
      ...
      || !identifier(message.promotionSite.email || message.profile.email)
