...
import { ensureLlmApiPermission } from './llm-options-controller.mjs';
  ...
  return normalize({
    ...
    preferences: {
      showExportOutlinksFloatingButton:
        values.show_export_outlinks_floating_button !== false
    }
  });
}

export function createSafeOptionsSettingsAdapter(
  syncStorage,
  { permissions } = {}
) {
  ...
  return Object.freeze({
    ...
    async save(settings) {
      ...
      const current = settingsFromStorage(
        await syncStorage.get(PORTABLE_SYNC_KEYS)
      );
      if (current.llm.apiBaseUrl !== llm.apiBaseUrl) {
        await ensureLlmApiPermission(permissions, llm.apiBaseUrl);
      }
