...
function retentionFingerprint(summary, now) {
  const oldestSubmittedAt = summary?.oldestSubmittedAt;
  return {
    stage: classifyRetentionAge(oldestSubmittedAt, now),
    oldestSubmittedAt: isTimestamp(oldestSubmittedAt) ? oldestSubmittedAt : null,
    dueSoonCount: Number(summary?.dueSoonCount) || 0,
    expiredCount: Number(summary?.expiredCount) || 0
  };
}

function sameRetentionFingerprint(left, right) {
  return Boolean(left && right)
    && left.stage === right.stage
    && left.oldestSubmittedAt === right.oldestSubmittedAt
    && left.dueSoonCount === right.dueSoonCount
    && left.expiredCount === right.expiredCount;
}
...
export function installCommentHistoryRetention(chromeApi, service, {
  ...
  async function runLocalReminderCheck(checkedAt) {
    ...
    const fingerprint = retentionFingerprint(summary, checkedAt);
    const sameFingerprint = sameRetentionFingerprint(
      reminderMeta?.fingerprint,
      fingerprint
    );
    const lastReminderAt = sameFingerprint ? reminderMeta?.lastReminderAt : undefined;

    const shouldNotify = shouldNotifyRetention({
      oldestSubmittedAt,
      lastReminderAt,
      now: checkedAt
    });
    if (!shouldNotify) {
      await service.setMeta(COMMENT_HISTORY_RETENTION_META_KEY, {
        lastReminderAt: sameFingerprint ? reminderMeta?.lastReminderAt ?? null : null,
        oldestSubmittedAt: isTimestamp(oldestSubmittedAt) ? oldestSubmittedAt : null,
  ...
  async function runRetentionCheck() {
    const checkedAt = now();
    let cloudStatus;
    if (typeof service.getCloudSyncStatus === 'function') {
      try {
        cloudStatus = await service.getCloudSyncStatus();
      } catch {
        return runLocalReminderCheck(checkedAt);
      }
    }
    if (cloudStatus?.enabled === true) {
      const evicted = await service.evictSyncedCacheBefore({
        vaultId: cloudStatus.vaultId,
        cutoff: checkedAt - EXPIRY_DAY * DAY_MS
      });
