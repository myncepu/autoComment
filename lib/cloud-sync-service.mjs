...
export function createCloudSyncService({
  ...
  async function performBootstrapImportedVault(credentials, transport) {
  ...
  function bootstrapImportedVault(credentials, transport) {
    const existing = bootstrapInFlightByVault.get(credentials.vaultId);
    if (existing) return existing;
    const flight = performBootstrapImportedVault(
      credentials,
      transport
    ).finally(() => {
      if (bootstrapInFlightByVault.get(credentials.vaultId) === flight) {
        bootstrapInFlightByVault.delete(credentials.vaultId);
      }
    });
    bootstrapInFlightByVault.set(credentials.vaultId, flight);
    return flight;
  }
