...
export function createDomainConfigRepository(storageArea, { now = Date.now } = {}) {
  ...
  function deleteProfile(profileId) {
    return enqueue(async () => {
      ...
      if (current.assignmentPolicy.pairs.some((pair) => pair.profileId === id)) {
        throw codedError('profile_in_use');
      }
  ...
  function deletePromotionSite(siteId) {
    return enqueue(async () => {
      ...
      if (current.assignmentPolicy.pairs.some((pair) => pair.promotionSiteId === id)) {
        throw codedError('promotion_site_in_use');
      }
      return writeCandidate({
        ...current,
