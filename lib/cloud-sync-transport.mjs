import { CLOUD_SYNC_PROTOCOL_VERSION } from './cloud-sync-protocol.mjs';
...
export function createCloudSyncTransport({
  ...
  return Object.freeze({
    status(deviceId) {
      return request(`/v1/status${queryString({
        deviceId,
        protocolVersion: CLOUD_SYNC_PROTOCOL_VERSION
      })}`, { method: 'GET' });
    },
    ...
    pull(query) {
      return request(`/v1/sync/pull${queryString(versionedQuery(query))}`, {
        method: 'GET'
      });
    },
    bootstrap(query) {
      return request(`/v1/sync/bootstrap${queryString(versionedQuery(query))}`, {
        method: 'GET'
      });
    },
    history(query) {
