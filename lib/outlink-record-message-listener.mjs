...
function route(message, repository) {
  switch (message.type) {
    ...
    case OUTLINK_MESSAGE_TYPES.EXPORT:
      return repository.exportRecords(message.filter);
...
export function installOutlinkRecordMessageListener(chromeApi, repositoryPromise) {
  const listener = (message, sender, sendResponse) => {
    ...
    const operation = message.type === OUTLINK_MESSAGE_TYPES.OPEN_PAGE
      ...
      : Promise.resolve(repositoryPromise)
          .then((repository) => route(message, repository));
