...
export class BatchTabManager {
  constructor({
    ...
    this.ignoreUnexpectedClose = false;
    this.batchId = null;
  ...
  async create(task) {
    ...
    const activity = {
      ...
      windowId,
      startTime: this.now(),
      ...(createdTab.backgroundCheckpointed === true
        ? {
            url: createdTab.url,
            backgroundCheckpointed: true,
            runtimeCheckpoint: createdTab.runtimeCheckpoint
          }
        : {})
    };
  ...
  handleRemoved(tabId) {
    ...
    if (this.ignoreUnexpectedClose) return;
    this.onUnexpectedClose(activity);
  }

  forgetRemoved(tabId) {
    const activity = this.byTabId.get(tabId) || null;
    this.expectedTabIds.delete(tabId);
    if (activity) this.removeMappings(activity);
    return activity;
  }
  ...
  quiesce() {
    this.ignoreUnexpectedClose = true;
  }
