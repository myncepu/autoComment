...
Task 13: complete and independently approved (local five-URL fixture, bounded OpenAI-compatible stub, deterministic delays; focused 10/10, full suite 528/528; no P0–P3 findings).
Task 14 ordinary-page Chrome acceptance: complete for 1440/1024/640 layouts, wizard shell, queue filter/details/focus, safe pause/resume, irreversible stop, retry, manual resolution, and offline/recovery/error/empty states. Evidence is under `docs/qa/`.
Task 14 extension-host smoke: same-window concurrency 3 was observed after adding the required `tabs` permission. A later third-party diagnostic exposed that the owned BATCH_HANDLE path ignored `autoSubmit: false`; remaining workers were stopped and the remote outcome is not asserted. Final acceptance must use only the local five-target fixture after the handle/automation fix.
