...
与冻结 handle 一致。密码输入框已填写，但密码值没有进入 handle、提交
记录、浏览器状态报告或本文档。
...
关闭使用 10 秒有界等待，并在之后强制关闭 fixture server 的活动连接。

回归测试覆盖了永不 settle 的 `context.close()`，后续真实 Chrome 运行的
`browserCleanup` 为 `closed`，进程正常以状态 0 退出。
